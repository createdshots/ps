...
# Edited by: Jack Tolley and Stephane Dorricott
##########
...
foreach ($process in $processnames) {
    Write-Host "Stopping Process $process. (Lenovo Only)"
...
foreach ($app in $apps) {
    ...
    # Uninstall Lenovo Vantage (takes a registry edit cause its weird)
...
start-process "D:\McAfee\Mccleanup.exe" -ArgumentList "-p StopServices,MFSY,PEF,MXD,CSP,Sustainability,MOCP,MFP,APPSTATS,Auth,EMproxy,FWdiver,HW,MAS,MAT,MBK,MCPR,McProxy,McSvcHost,VUL,MHN,MNA,MOBK,MPFP,MPFPCU,MPS,SHRED,MPSCU,MQC,MQCCU,MSAD,MSHR,MSK,MSKCU,MWL,NMC,RedirSvc,VS,REMEDIATION,MSC,YAP,TRUEKEY,LAM,PCB,Symlink,SafeConnect,MGS,WMIRemover,RESIDUE -v -s"
write-host "bada da dada"
...
Write-Output "Editing Registry to prevent OneDrive ever opening again"
Start-Sleep 5
Write-Output "Just kidding lol"
